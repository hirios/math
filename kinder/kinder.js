/* Kindergarten Activities Page JavaScript - Meadow Math */

/**
 * Kindergarten page controller
...
const KinderPage = {
  ...
  /**
   * Initialize the Kindergarten page
   */
  async init() {
  ...
  async loadData() {
    try {
      ...
      const response = await fetch(`${basePath}/data/kinder.json`);
      ...
      console.error('Error loading Kindergarten data:', error);
      this.renderError();
    }
  },
    ...
    return `
      <section class="level-section ${levelClass}" data-level="${levelNumber}">
        ...
        <div class="tabs-container">
          ...
          >
            ${this.renderActivitiesContent(level)}
          </div>
  ...
  /**
   * Render activities content (or empty state if no activities yet)
   */
  renderActivitiesContent(level) {
    if (!level.activities || level.activities.length === 0) {
      return `
        <div class="empty-state">
          <div class="empty-state-icon">🌱</div>
          <p class="empty-state-text">Activities coming soon!</p>
        </div>
      `;
    }
